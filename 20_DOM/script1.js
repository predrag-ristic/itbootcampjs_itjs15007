...
link.href = `https://www.google.com/webhp?hl=en`;
link.target = `_blank`;
link.id = `link`;
...
// link.style = `color: green; border: 5px solid orange; font-size: 24px`;

link.setAttribute(`style`,`color: magenta; border: 4px solid cyan; font-size: 24px`);
...
naslov1.innerHTML = `Naslov kreiran iz JS-a`;
document.body.prepend(naslov1) // Dodaje se kao prvi potomak

let naslov2 = document.createElement(`h1`);
naslov2.innerHTML = `Naslov kreiran iz JS-a`;
document.body.appendChild(naslov2) // Dodaje se kao poslednji potomak
